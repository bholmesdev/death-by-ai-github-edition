...
**Verdict** — The judge's narrative judgment of a response. Posted as a single comment on the response issue: exactly three cinematic sentences using the player's first name (e.g. "Jeff activates their super suit just in time…"), followed by a standalone footer line `( {emoji} {name} survived )` or `( {emoji} {name} died )`. The projector splits the comment on sentence boundaries for press-to-advance reveal; the footer is the final press. Tone is cinematic-warm, not snarky. Calibrated around ~50% survival rate.
